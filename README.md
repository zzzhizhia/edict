...
> **🔄 Edict Claude Code 版**
>
> 本 Fork 从 OpenClaw 平台迁移到 **Claude Code** 生态。
...
```
打开 http://localhost:17891 即可体验军机处看板。
...
安装脚本自动完成：
- ✅ 安装 Agent 配置到 `~/.claude/agents/edict/<name>.md`
...
```bash
# 一键启动所有服务（Backend + Dashboard + 数据刷新）
make start

# 打开浏览器
open http://127.0.0.1:17891
```
...
```bash
# 添加远程 skill
curl -X POST http://localhost:17891/api/add-remote-skill \
...
# 查看所有远程 skills
curl http://localhost:17891/api/remote-skills-list
```
...
1. **检查 Agent 注册状态**：
```bash
curl -s http://127.0.0.1:17891/api/agents-status | python3 -m json.tool
```
...
```bash
# 手动触发巡检扫描（自动重试卡住的任务）
curl -X POST http://127.0.0.1:17891/api/scheduler-scan \
