...
### ⚡ 接任务时（必须立即执行）
```bash
python3 ${EDICT_HOME:-$HOME/.claude/edict}/scripts/kanban_update.py state JJC-xxx Doing "户部开始执行[子任务]"
python3 ${EDICT_HOME:-$HOME/.claude/edict}/scripts/kanban_update.py flow JJC-xxx "户部" "户部" "▶️ 开始执行：[子任务内容]"
```

### ✅ 完成任务时（必须立即执行）
```bash
python3 ${EDICT_HOME:-$HOME/.claude/edict}/scripts/kanban_update.py flow JJC-xxx "户部" "尚书省" "✅ 完成：[产出摘要]"
```
...
python3 ${EDICT_HOME:-$HOME/.claude/edict}/scripts/kanban_update.py flow JJC-xxx "户部" "尚书省" "🚫 阻塞：[原因]，请求协助"
```
...
```bash
# 开始分析
python3 ${EDICT_HOME:-$HOME/.claude/edict}/scripts/kanban_update.py progress JJC-xxx "正在收集数据源，确定统计口径" "数据收集🔄|数据清洗|统计分析|生成报表|提交成果"

# 分析中
python3 ${EDICT_HOME:-$HOME/.claude/edict}/scripts/kanban_update.py progress JJC-xxx "数据清洗完成，正在进行聚合分析" "数据收集✅|数据清洗✅|统计分析🔄|生成报表|提交成果"
```
